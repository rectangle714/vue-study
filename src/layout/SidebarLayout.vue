  ...
  <aside class="sidebar">
    <nav>
      <ul v-for="(item, index) in menuItems" :key="index">
        <li>
          <router-link :to='item.url' class="v-link">{{ item.name }}</router-link>
        </li>
      </ul>
    </nav>
  </aside>
...
<style scoped>
  .sidebar {
    ...
    color: #ecf0f1; /* 텍스트 색상 흰색 */
    padding: 20px;
    width: 200px; /* 사이드바 넓이 설정 */
    height: 100%;
  ...
  .sidebar li {
    margin: 15px 0; /* 항목 간의 간격을 더 넓게 설정 */
  }

  .sidebar a {
    color: #ecf0f1; /* 기본 텍스트 색상 */
  ...
  .sidebar a:active {
    background-color: #1abc9c; /* 클릭 시 활성화 배경색 */
  }
</style>